...
type DoctorAvailability = typeof schema.doctorAvailability.$inferSelect;
type InsertDoctorAvailability = typeof schema.doctorAvailability.$inferInsert;
  ...
  try {
    ...
    if (fs.existsSync(migrationsDir)) {
      ...
      const appliedRows = sqlite
        ...
        .all()
        .map((row: any) => row.id);
      const appliedSet = new Set<string>(appliedRows);
...
export interface IStorage {
  ...
  deletePathologyCategoryTest(id: string): Promise<boolean>;
    ...
    upsertDoctorAvailability(
      data: InsertDoctorAvailability,
    ): Promise<DoctorAvailability>;
    deleteDoctorAvailability(id: string): Promise<void>;

    // Appointment Management
    createAppointment(
      appointment: Omit<InsertAppointment, "endDatetime">,
    ): Promise<Appointment>;
...
export class SqliteStorage implements IStorage {
  ...
  async getDoctorAvailability(doctorId: string): Promise<DoctorAvailability[]> {
    return db
      ...
      .orderBy(
        schema.doctorAvailability.weekday,
        schema.doctorAvailability.startTime,
      )
      .all();
  }
  ...
  async upsertDoctorAvailability(
    ...
    const availabilityId = data.id || this.generateId();
    const overlapConditions = [
      eq(schema.doctorAvailability.doctorId, data.doctorId),
      eq(schema.doctorAvailability.weekday, data.weekday),
      eq(schema.doctorAvailability.isActive, true),
      lt(schema.doctorAvailability.startTime, data.endTime),
      gt(schema.doctorAvailability.endTime, data.startTime),
      ne(schema.doctorAvailability.id, availabilityId),
    ];

    const overlapping = db
      .select({ id: schema.doctorAvailability.id })
      .from(schema.doctorAvailability)
      .where(and(...overlapConditions))
      .get();

    if (overlapping) {
      throw new Error("Availability time overlaps with existing range");
    }

    if (data.id) {
      const updated = db
        .update(schema.doctorAvailability)
        .set({ ...data, updatedAt: new Date().toISOString() })
        .where(eq(schema.doctorAvailability.id, data.id))
        .returning()
        .get();
    ...
    return db
      .insert(schema.doctorAvailability)
      .values({ ...data, id: availabilityId })
      .returning()
      .get();
  }
  ...
  async deleteDoctorAvailability(id: string): Promise<void> {
    await db
      .delete(schema.doctorAvailability)
      .where(eq(schema.doctorAvailability.id, id))
      .run();
  }

  async createAppointment(
    appointment: Omit<InsertAppointment, "endDatetime" | "durationMinutes">,
  ): Promise<Appointment> {
    const durationConfig = db
      .select({
        durationMinutes: schema.doctorServiceDurations.durationMinutes,
      })
      .from(schema.doctorServiceDurations)
      .where(
        and(
          eq(schema.doctorServiceDurations.doctorId, appointment.doctorId),
          eq(
            schema.doctorServiceDurations.serviceType,
            appointment.sourceType,
          ),
          eq(schema.doctorServiceDurations.isActive, true),
        ),
      )
      .get();
    ...
    if (!durationConfig) {
      throw new Error(
        "No duration configured for this doctor and service type",
      );
    }

    const startDateTime = new Date(appointment.startDatetime);
    const durationMs = durationConfig.durationMinutes * 60 * 1000;
    const endDateTime = new Date(startDateTime.getTime() + durationMs);
    const computedEndDatetime = endDateTime.toISOString();
    ...
    const availability = db
      ...
      .where(
        and(
          eq(schema.doctorAvailability.doctorId, appointment.doctorId),
          eq(schema.doctorAvailability.weekday, weekday),
          eq(schema.doctorAvailability.isActive, true),
          lte(schema.doctorAvailability.startTime, startTime),
          gte(schema.doctorAvailability.endTime, endTime),
        ),
      )
      .get();
    ...
    if (appointment.doctorId) {
      const doctorOverlap = db
        .select({ id: schema.appointments.id })
        .from(schema.appointments)
        .where(
          and(
            eq(schema.appointments.doctorId, appointment.doctorId),
            eq(schema.appointments.status, "scheduled"),
            lt(schema.appointments.startDatetime, computedEndDatetime),
            gt(schema.appointments.endDatetime, appointment.startDatetime),
          ),
        )
        .get();
    ...
    const patientOverlap = db
      .select({ id: schema.appointments.id })
      .from(schema.appointments)
      .where(
        and(
          eq(schema.appointments.patientId, appointment.patientId),
          eq(schema.appointments.status, "scheduled"),
          lt(schema.appointments.startDatetime, computedEndDatetime),
          gt(schema.appointments.endDatetime, appointment.startDatetime),
        ),
    ...
    if (patientOverlap) {
      throw new Error("Patient already has an appointment in this time range");
    }

    return db
      .insert(schema.appointments)
      .values({ ...appointment, endDatetime: endDateTime.toISOString() })
      .returning()
      .get();
  }
